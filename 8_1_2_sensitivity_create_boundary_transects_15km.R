...
bd.inner.buffer <- map(bd.subset, function(x){st_buffer(x, -buffer_dist)}) #4500m inside boundary
...
bd.pts.w.info <- rbind(bd.pts.w.info1, bd.pts.w.info2 %>% dplyr::select(-WDPAID), bd.pts.w.info3 %>% dplyr::select(-WDPAID), bd.pts.w.info4 %>% dplyr::select(-WDPAID)) %>%
     dplyr::select(-Join_Count, -TARGET_FID, -JOIN_FID)

nrow(bd.pts.w.info) # 2,189,243
length(unique(bd.pts.w.info$WDPA_PID))  #6,132 PAs original, 5009 now. 
...
nrow(bd.pts.w.info.df) #1,686,993
...
nrow(transect.join) # 5,060,979  (filtered from 5,795,625)
...
nrow(transect.pts.w.info) # 5,060,979
...
transect.pts <- transect.pts.w.info %>% 
  ...
  st_set_crs(target.crs)
nrow(transect.pts) # 5,060,979
length(unique(transect.pts$WDPA_PID)) # 4223
...
st_write(transect.pts[(4000001:5060979),], paste0("./result/sensitivity_analysis/gee_transects_", transect_unit*transect_pts*2/1000, "_pt5.shp"))
#------------------------------------------------------
# END CODE
#------------------------------------------------------
